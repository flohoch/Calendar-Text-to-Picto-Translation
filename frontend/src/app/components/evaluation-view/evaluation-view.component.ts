...
@Component({
  ...
  template: `
    <div class="eval-view">
      ...
      <ul class="run-list" *ngIf="runs.length > 0; else noRuns">
        <li *ngFor="let r of runs" (click)="loadRun(r.filename)" [class.active]="selectedFilename === r.filename">
      ...
      <div *ngIf="selectedRun" class="run-detail">
        ...
        <div class="metrics-grid">
          ...
          <div class="metric"><label>Accuracy (Summary)</label><span class="value">{{ pct(selectedRun.metrics.accuracy_summary) }}</span></div>
          <div class="metric"><label>Accuracy (Location)</label><span class="value">{{ pct(selectedRun.metrics.accuracy_location) }}</span></div>
          <div class="metric"><label>Accuracy (Attendees)</label><span class="value">{{ pct(selectedRun.metrics.accuracy_attendees) }}</span></div>
          <div class="metric"><label>Accuracy (Overall)</label><span class="value">{{ pct(selectedRun.metrics.accuracy_overall) }}</span></div>
        ...
        <div class="entries">
          <div class="entry" *ngFor="let e of selectedRun.entries">
            <div class="entry-header">
              ...
              <span *ngIf="e.summary_correct !== null" [class.correct]="e.summary_correct" [class.wrong]="!e.summary_correct">
                summary: {{ e.summary_correct ? '✓' : '✗' }}
              </span>
              <span *ngIf="e.location_correct !== null" [class.correct]="e.location_correct" [class.wrong]="!e.location_correct">
                location: {{ e.location_correct ? '✓' : '✗' }}
              </span>
              <span *ngIf="e.attendees_correct !== null" [class.correct]="e.attendees_correct" [class.wrong]="!e.attendees_correct">
  ...
  styles: [
    `
      ...
      .hint { color: #777; font-size: 0.9rem; margin-bottom: 1rem; }
      .run-controls {
      ...
      .run-list li {
        ...
        border-radius: 8px; cursor: pointer; box-shadow: 0 1px 4px rgba(0,0,0,0.05);
